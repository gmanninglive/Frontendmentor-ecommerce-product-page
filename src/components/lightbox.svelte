...
<div class={`flex justify-center items-center absolute top-0 left-0 z-10 w-full h-full bg-black/[0.75] ${isOpen ? "block ": "hidden "} `}>
    <div class="w-1/2 max-w-[500px]">
        <div class="relative">
            <button on:click={() => isOpen = true}>
                <img src={images[selected]} alt="selected" class="w-full rounded-lg" />
            </button>
            <button on:click={() => {if(selected > 1) selected--}} class="absolute top-1/2 bottom-1/2 h-10 w-10 -left-5 hover:opacity-50 p-4 rounded-full bg-white flex justify-center items-center"> <img src="/images/icon-previous.svg" alt="previous" /> </button>
            <button on:click={() => {if(selected < 4) selected++}} class="absolute top-1/2 bottom-1/2 h-10 w-10 -right-5 hover:opacity-50 p-4 rounded-full bg-white flex justify-center items-center"> <img src="/images/icon-next.svg" alt="next" /> </button>
            <button on:click={() => isOpen = false} class="absolute -top-4 -right-4 "> <img src="/images/icon-close.svg" alt="close" /> </button>
