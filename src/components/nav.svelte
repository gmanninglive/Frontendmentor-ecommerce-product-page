...
<nav
	class="py-10 w-full flex justify-between items-center gap-x-12
...
>
	...
	<div class="flex gap-x-6 items-center">
		...
		<button on:click={() => cartvisible = !cartvisible} class="relative focus:text-orange">
           ...
           {#if cartSize > 0}
            <p class="absolute -top-2 -right-2 text-orange text-xs bg-gray-600 shadow-md rounded-full px-1 z-10">{cartSize}</p>
            {/if}
		</button>
